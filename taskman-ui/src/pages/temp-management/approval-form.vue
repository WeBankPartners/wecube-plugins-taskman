...
export default {
  ...
  methods: {
    ...
    loadPage (id = '') {
      ...
      // this.cancelGroup()
      this.getApprovalNodeX(id)
      this.getAllDataModels()
    },
    async getApprovalNodeX (id = '') {
    ...
    async addApprovalNode (sort) {
      ...
      if (nodeStatus === 'canSave') {
        ...
        if (statusCode === 'OK') {
          this.getApprovalNodeX(data.taskTemplate.id)
        }
      }
    ...
    beforeEditNode (node) {
      ...
      //   console.log(9.5)
      //   this.loadPage(this.activeEditingNode.id)
      //   return
      // }
      this.saveGroup(3, node)
    },
