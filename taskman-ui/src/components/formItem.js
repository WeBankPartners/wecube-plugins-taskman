...
export default {
  ...
  render () {
    const { elementType, width, title, options, value, defaultValue, placeholder,isHover, isActive, entity, isCustom, isMultiple } = this.item
    return (
      <Col id={'formItem_'+this.index} span={width}>
        <FormItem class={this.classList} label={title}>
          <elementType options={options} isMultiple={isMultiple} value={this.itemValue} onInput={v => this.inputHandler(v)} placeholder={placeholder}></elementType>
