...
package: image
	...
	zip  wecube-plugins-taskman-$(version).zip image.tar init.sql practices_structure.sql practices_demo_data.sql register.xml docker-compose.yml default.json ui.zip
...
upload: package
	...
	docker exec $(container_id) mc cp /package/wecube-plugins-taskman-$(version).zip wecubeS3/wecube-plugin-package-bucket
	docker rm -f $(container_id)
	rm -rf wecube-plugins-taskman-$(version).zip