...
func hasProcessedTaskSQL(queryTimeStart, queryTimeEnd string, templateType int, user string, taskType models.TaskType) (sql string, queryParam []interface{}) {
	...
	if taskType == models.TaskTypeNone {
		...
		queryParam = append(queryParam, templateType, models.TaskTypeSubmit, user, queryTimeStart, queryTimeEnd)
	} else {
		sql = "select * from (select t.id,t.request,t.template_type,t.name,t.type,t.created_time as task_created_time,th.updated_time as task_approval_time,t.updated_time,t.status,t.expire_time,th.role as task_handle_role,th.id as task_handle_id,th.handler as task_handler,t.del_flag,th.latest_flag,th.handle_status,th.handle_result,th.created_time as task_handle_created_time,th.updated_time as task_handle_updated_time,t.request_created_time from task t right join task_handle th ON t.id = th.task) tha where del_flag = 0 and handle_result is not null and template_type = ? and type = ? and task_handler =? and latest_flag = 1  and request_created_time >= ? and request_created_time <= ?"
		queryParam = append(queryParam, templateType, taskType, user, queryTimeStart, queryTimeEnd)
	}
	return
}
...
func submitSQL(rollback, templateType int, queryTimeStart, queryTimeEnd, user string) (sql string, queryParam []interface{}) {
	...
	queryParam = []interface{}{user, templateType, queryTimeStart, queryTimeEnd}
	return
}
...
func getPlatData(req models.PlatDataParam, newSQL, language string, page bool) (pageInfo models.PageInfo, rowsData []*models.PlatformDataObj, err error) {
	...
	if len(rowsData) > 0 {
		...
		for _, platformDataObj := range rowsData {
			...
			// 获取当前节点和计算请求进度
			platformDataObj.Progress, platformDataObj.CurNode = CalcRequestProgressAndCurNode(platformDataObj.Id, platformDataObj.TaskId, platformDataObj.ProcInstanceId, req.UserToken, language)
			...
			platformDataObj.HandleRole, platformDataObj.Handler = getRequestHandler(models.RequestTable{Id: platformDataObj.Id, Status: platformDataObj.Status, RequestTemplate: platformDataObj.TemplateId, CreatedBy: platformDataObj.CreatedBy, Role: platformDataObj.Role}, platformDataObj.TaskId)
...
func CalcRequestProgressAndCurNode(requestId, taskId, instanceId, userToken, language string) (progress int, curNode string) {
...
func getRequestForm(request *models.RequestTable, taskId, userToken, language string) (form models.RequestForm) {
	if request == nil {
		return
	}
	...
	form.Progress, form.CurNode = CalcRequestProgressAndCurNode(request.Id, taskId, request.ProcInstanceId, userToken, language)
...
func getRequestHandler(request models.RequestTable, taskId string) (role, handler string) {
	var task *models.TaskTable
	var taskHandleList []*models.TaskHandleTable
	...
	task, _ = GetTaskService().GetDoingTask(request.Id, request.RequestTemplate, taskId)
	if task != nil {
		// 根据任务查询 任务处理人
