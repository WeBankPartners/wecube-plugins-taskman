	...
	for _, v := range taskTable {
		...
		if (tmpExpireObj.Percent >= 75) && (doingNotifyCount == 0) || ((tmpExpireObj.Percent >= 100) && (timeoutNotifyCount == 0)) {
			...
			if len(taskNotifyList) > 0 {
				...
				actions = append(actions, &dao.ExecAction{Sql: "insert into task_notify(id,task,doing_notify_count,timeout_notify_count,err_msg,updated_time)values(?,?,?,?,?,?)", Param: []interface{}{guid.CreateGuid(), v.Id, tmpExpireObj.DoingNotifyCount, tmpExpireObj.TimeoutNotifyCount, errMsg, time.Now().Format(models.DateTimeFormat)}})
			}
		}
	}
	if len(actions) > 0 {
