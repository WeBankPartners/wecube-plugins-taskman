/***
 * 重写swagger-bootstrap-ui 实现显示接口权限
...
(function ($) {
    ...
    SwaggerBootstrapUi.prototype.initSettings = function () {
        ...
        if (that.requestParameter != null) {
            ...
            //菜单权限显示
            if (checkFiledExistsAndEqStr(that.requestParameter, "showAuth", "1")) {
                that.settings.showAuth = true;
            }
                    ...
                    $.each(newTagArrs, function (i, tag) {
                        ...
                        if (len == 0) {
                            ...
                            $.each(tag.childrens, function (i, children) {
                                ...
                                var _spanHtml = '<span class="swu-menu swu-left"><span class="menu-url{addClass}"'+ depStyle +'>{value}</span></span>';
                                var _liHtml ='<li class="menuLi" lay-id="' + tabId + '" >' + newApiIcon +'<div class="mhed">{info}{url}{autu}</div></li>';
                                var _infoHtml ='<div class="swu-hei">' +
                                    _spanHtml.replace("{addClass}","-"+children.methodType.toLowerCase()).replace("{value}",children.methodType.toUpperCase())+
                                    _spanHtml.replace("{addClass}","").replace("value",children.summary)+'</div>';
                                var _urlHtml = '<div class="swu-hei">' +
                                    _spanHtml.replace("{addClass}","-"+children.methodType.toLowerCase()).replace("{value}","URL")+
                                    _spanHtml.replace("{addClass}"," swu-menu-api-des").replace("{value}",children.showUrl)+'</div>';
                                var _authHtml = '<div class="swu-hei">' +
                                    _spanHtml.replace("{addClass}","-"+children.methodType.toLowerCase()).replace("{value}","URL")+
                                    _spanHtml.replace("{addClass}"," swu-menu-api-des")
                                        .replace("{value}",children.auth ? "<span style='color: #FD482C;'>需要登录</span>" :"默认")+'</div>';

                                _liHtml = _liHtml.replace("{info}",_infoHtml);
                                _liHtml = _liHtml.replace("{url}",that.settings.showApiUrl ? _urlHtml :'');
                                _liHtml = _liHtml.replace("{url}",that.settings.showApiUrl ? _authHtml :'');
                                childrenLi =$(_liHtml);
                                childrenLi.data("data", children);
                                ul.append(childrenLi);
                            })
            ...
            if (len == 0) {
                ...
                if (that.settings.showTagStatus) {
                    ...
                    titleA = $(
                        ...
                        '<i class="icon-file-alt icon-text-width iconfont icon-APIwendang"></i>' +
                        '<span class="menu-text sbu-tag-description"> ' + tag.name + "(" + tag.description + ')<span class="badge badge-primary ">' + len + '</span></span>' + tagNewApiIcon + '<b class="arrow icon-angle-down"></b></a>');
                } else {
                    _lititle = tag.name;
                    titleA = $('<a href="#" class="dropdown-toggle"><i class="icon-file-alt icon-text-width iconfont icon-APIwendang"></i><span class="menu-text"> ' + tag.name + '<span class="badge badge-primary ">' + len + '</span></span>' + tagNewApiIcon + '<b class="arrow icon-angle-down"></b></a>');
                }
                ...
                $.each(tag.childrens, function (i, children) {
                    ...
                    var tabId = "tab" + children.id;
                    var summary = (i+1)+"、"+children.summary;
                    var _hashUrl = children.hashCollections[0];
                    var menuUrlClass = 'menu-url-'+children.methodType.toLowerCase();
                    var _spanHtml = '<span class="swu-menu swu-left"><span class="menu-url{addClass}"'+ depStyle +'>{value}</span></span>';
                    var _liHtml ='<li class="menuLi"  lay-id="' + tabId + '" data-hashurl="' + _hashUrl + '">' + newApiIcon +'<div class="mhed">{info}{url}{auth}</div></li>';
                    // 接口信息 html
                    var _infoHtml ='<div class="swu-hei">' +
                        _spanHtml.replace("{addClass}","-"+children.methodType.toLowerCase()).replace("{value}",children.methodType.toUpperCase())+
                        _spanHtml.replace("{addClass}","").replace("{value}",summary)+'</div>';
                    // URL信息 html
                    var _urlHtml = '<div class="swu-hei">' +
                        _spanHtml.replace("{addClass}","-"+children.methodType.toLowerCase()).replace("{value}","URL")+
                        _spanHtml.replace("{addClass}"," swu-menu-api-des").replace("{value}",children.showUrl)+'</div>';
                    // 权限信息 html
                    var _authHtml = '<div class="swu-hei">' +
                        _spanHtml.replace("{addClass}","-"+children.methodType.toLowerCase()).replace("{value}","权限")+
                        _spanHtml.replace("{addClass}"," swu-menu-api-des")
                            .replace("{value}",children.auth ? "<span style='color: #FD482C;'>需要登录访问</span>" :"<span style='color: #5FB878;'>可以直接访问</span>")+'</div>';
                    _liHtml = _liHtml.replace("{info}",_infoHtml);
                    _liHtml = _liHtml.replace("{url}",that.settings.showApiUrl ? _urlHtml :'');
                    _liHtml = _liHtml.replace("{auth}",that.settings.showAuth  ? _authHtml :'');
                    // _liHtml = _liHtml.replace("{auth}", _authHtml);
                    childrenLi =$(_liHtml);
                    childrenLi.data("data", children);
                    ul.append(childrenLi);
                })
    ...
    SwaggerBootstrapUi.prototype.settingsCallback = function (flag) {
        ...
        var showAuth = $("#SwaggerBootstrapUiSettings").find("input[name=showAuth]");
        var langEle = $("#SwaggerBootstrapUiSettings").find("input[name=language]:checked");
        ...
        var showAuthFlag = showAuth.prop("checked");
        var enableSbuFlag = enableSbu.prop("checked");
    ...
    SwaggerBootstrapUi.prototype.createApiInfoInstance = function (path, mtype, apiInfo) {
        ...
        if (apiInfo != null) {
            ...
            //读取扩展属性is-auth FIXME add
            if (apiInfo.hasOwnProperty("is-auth")) {
                swpinfo.auth = apiInfo["is-auth"];
            }
            //operationId
