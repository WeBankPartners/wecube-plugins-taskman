...
func NotifyTaskExpireMail(task *models.TaskTable, expireObj *models.ExpireObj, userToken, language string) (err error) {
	...
	if err = dao.X.SQL("select * from task_handle where task = ?", task.Id).Find(&taskHandleList); err != nil {
		log.Error(nil, log.LOGGER_APP, "NotifyTaskExpireMail query db err", zap.Error(err))
		return
	}
	...
	for _, taskHandle := range taskHandleList {
		if taskHandle.Handler != "" {
			...
			if userInfo == nil || strings.TrimSpace(userInfo.Email) == "" {
				log.Warn(nil, log.LOGGER_APP, "NotifyTaskExpireMail,taskName receiver email is empty", zap.String("requestName", requestName), zap.String("taskName", task.Name), zap.String("receiver", taskHandle.Handler))
				return
			}
	...
	log.Debug(nil, log.LOGGER_APP, "NotifyTaskExpireMail", zap.String("mailSubject", mailSubject), zap.String("mailContent", mailContent), zap.String("mailList", strings.Join(mailList, ",")))
	if err = models.MailSender.Send(mailSubject, mailContent, mailList); err != nil {
		log.Error(nil, log.LOGGER_APP, "send notify email fail", zap.Error(err))
		return
	}
	return nil
}
...
func NotifyTaskRoleMail(requestName, taskName, expireDate, role, userToken, language string) (err error) {
	...
	if len(mailList) > 0 {
		log.Debug(nil, log.LOGGER_APP, "NotifyTaskRoleMail", zap.String("mailSubject", subject), zap.String("mailContent", content), zap.String("mailList", mailList[0]))
...
func NotifyTaskAssignMail(requestName, taskName, expireDate, receiver, userToken, language string) (err error) {
	...
	if userInfo == nil || strings.TrimSpace(userInfo.Email) == "" {
		log.Warn(nil, log.LOGGER_APP, "NotifyTaskAssignMail,taskName receiver email is empty", zap.String("requestName", requestName), zap.String("taskName", taskName), zap.String("receiver", receiver))
		return
	}
	taskName = getInternationalizationTaskName(taskName, language)
	subject = "[wecube] [New Task reminder]  【新增任务提醒】"
	...
	content = content + fmt.Sprintf("\n%s/#/taskman/workbench", models.Config.WebUrl)
	log.Debug(nil, log.LOGGER_APP, "NotifyTaskAssignMail", zap.String("mailSubject", subject), zap.String("mailContent", content), zap.String("mailList", userInfo.Email))
...
func NotifyTaskHandlerUpdateMail(requestName, taskName, originHandler, targetHandler, userToken, language string) (err error) {
	...
	if userInfo == nil || strings.TrimSpace(userInfo.Email) == "" {
		log.Warn(nil, log.LOGGER_APP, "NotifyTaskHandlerUpdateMail,taskName receiver email is empty", zap.String("requestName", requestName), zap.String("taskName", taskName), zap.String("receiver", originHandler))
		return
	}
	taskName = getInternationalizationTaskName(taskName, language)
	...
	content = content + fmt.Sprintf("\n%s/#/taskman/workbench", models.Config.WebUrl)
	log.Debug(nil, log.LOGGER_APP, "NotifyTaskHandlerUpdateMail", zap.String("mailSubject", subject), zap.String("mailContent", content), zap.String("mailList", userInfo.Email))
	err = models.MailSender.Send(subject, content, []string{userInfo.Email})
	if err != nil {
		log.Error(nil, log.LOGGER_APP, "send mail err", zap.Error(err))
	}
...
func NotifyRequestCompleteMail(requestName, creator, userToken, language string) (err error) {
	...
	if userInfo == nil || strings.TrimSpace(userInfo.Email) == "" {
		log.Warn(nil, log.LOGGER_APP, "NotifyRequestCompleteMail,requestName creator email is empty", zap.String("requestName", requestName), zap.String("creator", creator))
		return
	}
	...
	content = content + fmt.Sprintf("\n%s/#/taskman/workbench", models.Config.WebUrl)
	log.Debug(nil, log.LOGGER_APP, "NotifyRequestCompleteMail", zap.String("mailSubject", subject), zap.String("mailContent", content), zap.String("mailList", userInfo.Email))
...
func NotifyTaskBackMail(requestName, taskName, creator, approval, userToken, language string) (err error) {
	...
	if userInfo == nil || strings.TrimSpace(userInfo.Email) == "" {
		log.Warn(nil, log.LOGGER_APP, "NotifyTaskBackMail,requestName creator email is empty", zap.String("requestName", requestName), zap.String("creator", creator))
		return
	}
	taskName = getInternationalizationTaskName(taskName, language)
...
func NotifyTaskDenyMail(requestName, taskName, creator, approval, userToken, language string) (err error) {
	...
	if userInfo == nil || strings.TrimSpace(userInfo.Email) == "" {
		log.Warn(nil, log.LOGGER_APP, "NotifyTaskDenyMail,requestName creator email is empty", zap.String("requestName", requestName), zap.String("creator", creator))
		return
	}
	taskName = getInternationalizationTaskName(taskName, language)
	...
	content = content + fmt.Sprintf("\n%s/#/taskman/workbench", models.Config.WebUrl)
	log.Debug(nil, log.LOGGER_APP, "NotifyTaskDenyMail", zap.String("mailSubject", subject), zap.String("mailContent", content), zap.String("mailList", userInfo.Email))
...
func NotifyTaskWorkflowFailMail(requestName, procDefName, status, creator, userToken, language string) (err error) {
	...
	if userInfo == nil || strings.TrimSpace(userInfo.Email) == "" {
		log.Warn(nil, log.LOGGER_APP, "NotifyTaskWorkflowTerminationMail,requestName creator email is empty", zap.String("requestName", requestName), zap.String("creator", creator))
		return
	}
	subject = "[wecube] [Request Termination Reminder]  【请求终止提醒】"
	...
	log.Debug(nil, log.LOGGER_APP, "NotifyTaskWorkflowFailMail", zap.String("mailSubject", subject), zap.String("mailContent", content), zap.String("mailList", userInfo.Email))
