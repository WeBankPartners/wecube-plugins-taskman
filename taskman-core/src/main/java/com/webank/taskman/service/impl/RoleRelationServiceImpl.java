...
@Service
public class RoleRelationServiceImpl extends ServiceImpl<RoleRelationMapper, RoleRelation>
        ...
        roles.stream().forEach(role -> {
            if (!StringUtils.isEmpty(role.getRoleName()) && !StringUtils.isEmpty(role.getDisplayName())) {
                this.getBaseMapper().insert(
                        new RoleRelation(recordId, roleType.getType(), role.getRoleName(), role.getDisplayName()));
            }
        });
    }
